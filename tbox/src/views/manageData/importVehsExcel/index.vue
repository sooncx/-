...
export default {
  ...
  methods: {
    ...
    async doGetErrorVehicleMessage() {
      ...
      if (!data.error.length && !data.right.length) return;
      ...
      try {
        ...
        //跳转检验数据
        this.step = 1;
        this.tableData = data.error;

        //获取导入车组
        const { organizationId, organizationName } = data.error[0];
        this.model.organization = { organizationId, organizationName };
        if (!data.error.length && data.right.length) {
          this.doImportExcelVehicle();
        }
      } catch (error) {
        console.error(error);
