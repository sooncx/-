export default class Trajectory {
  constructor({
    ...
    this.car.on("moving", ({ passedPath }) => {
      this.passedLine.setPath([
        ...this.trackPath.slice(0, this.passedIndex + 1),
        ...passedPath
      ]);
  ...
  setPath(path) {
    ...
    if (path.length) {
      ...
      endMarker.setPosition(path[path.length - 1]);
  ...
  jumpTo(passedIndex) {
    ...
    startIndex !== endIndex &&
      this.car.setAngle(
        Trajectory.TwoCoordinateAzimuth(
          this.trackPath[startIndex],
          this.trackPath[endIndex]
        )
      );
  ...
  play() {
    if (!this.trackPath.length) {
      console.error("请先设置轨迹数据");
      return;
    }

    if (this.passedIndex === this.trackPath.length - 1) {
      //到达终点后通知播放停止
      this.isPlay = false;
      return;
    }
