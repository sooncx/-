    ...
    <div class="box">
      ...
      >
        ...
        <ul
          ...
          :key="index"
        >
          <i :class="['iconfont icon-truck',
          {'onlineIcon':item.state === 0},
          {'offlineIcon':item.state === 1},
          {'noactiveIcon':item.state === 2},
          {'staticIcon':item.state === 3},
          {'sleepIcon':item.state === 4}
          ]"></i>
          <li
            @click="liClick(item)"
...
export default {
  ...
  methods: {
    ...
    async getVehicleMonitoringByOrganizationId(mydata) {
      ...
      this.nodeList = data.vehicleMonitoringVos || [];
      this.list = data.vehicleMonitoringVos
        ? data.vehicleMonitoringVos.slice(0, 20)
        : [];
      this.loading = false;
      console.log(data.vehicleMonitoringVos);
    },
    nodeClick(data) {
...
.onlineIcon {
  color: rgb(195, 110, 110) !important;
}
.offlineIcon {
  color: rgb(102, 172, 137) !important;
}
.staticIcon {
  color: rgb(73, 178, 197) !important;
}
.sleepIcon {
