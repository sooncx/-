...
export default {
  ...
  data() {
    return {
      ...
      activeIndex: null,
      checkTypeObj: {
        1: { key: "terminalNo", desc: "设备号" },
        2: { key: "sim", desc: "SIM卡号" },
        3: { key: "bmsNo", desc: "电池编号" }
      ...
      handler(val) {
        ...
        if (val.vehicleId !== undefined) {
          label =
            val.type === undefined
              ? val.terminalNo
              : val[this.checkTypeObj[val.type].key];
        } else {
  ...
  methods: {
    ...
    async getVehs(query) {
      const { data } = await this.$API.getVehicleByL({
        string: query
      });

      return data.map(item => {
        const { key, desc } = this.checkTypeObj[item.type];
        return {
          label: item[key],
          value: { ...item },
          desc
